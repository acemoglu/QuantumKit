...
struct GatePaletteView: View {
                    ...
                    ForEach(PaletteSection.allCases) { section in
                        HStack(spacing: 6) {
                            ...
                            ForEach(PaletteTool.allCases.filter { $0.section == section }) { tool in
                                PaletteChipView(tool: tool, isSelected: viewModel.selectedPaletteTool == tool)
                                    .draggable(tool) {
    ...
    private func pendingHint(_ pending: PendingGatePlacement) -> String {
        ...
        if pending.tool == .cx || pending.tool == .cz {
            return pending.pickedQubits.isEmpty ? "Drop/click control qubit" : "Now pick the target qubit"
        }
        return "Pick \(remain) more qubit(s) for \(pending.tool.title)"
    }
}
